...
async function main() {
  const fs = require('fs');
  const goldenSet = JSON.parse(fs.readFileSync('golden_test_set_v2.json', 'utf8'));
  const items = goldenSet.items;

  console.log(`[EP-10/v2] تشغيل كامل: ${items.length} سؤال، ENABLE_RERANK_VERIFICATION=true`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    fs.writeFileSync('golden_eval_v2_results.json', JSON.stringify(results, null, 2), 'utf8');
